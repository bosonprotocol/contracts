...
contract("Cashier withdrawals ", async addresses => {
  ...
  async function deployContracts () {
    ...
    contractVoucherKernel =
      await VoucherKernel.new(contractERC1155ERC721.address)
  ...
  async function giveAwayToRandom () {
    const balanceBuyerFromPayment = await contractBSNTokenPrice
      .balanceOf(users.buyer.address)
    const balanceBuyerFromDesosits = await contractBSNTokenDeposit
      .balanceOf(users.buyer.address)

    const balanceSellerFromPayment = await contractBSNTokenPrice
      .balanceOf(users.seller.address)
    const balanceSellerFromDesosits = await contractBSNTokenDeposit
      .balanceOf(users.seller.address)

    const escrowBalanceFromPayment = await contractBSNTokenPrice
      .balanceOf(users.deployer.address)
    const escrowBalanceFromDeposits = await contractBSNTokenDeposit
      .balanceOf(users.deployer.address)
  ...
  for (let i = 0; i <= PAUSED_WITH_PERMIT; i++) {
    describe('Withdraw scenarios', async () => {
      ...
      describe(`ETH - ETH${i === PAUSED_WITH_PERMIT ? PAUSED_LABEL : ''}`,
        async () => {
          before(async () => {
          ...
          it("COMMIT->REFUND->COMPLAIN->CANCEL->FINALIZE->WITHDRAW",
            async () => {
              // 0.3 + 0.04 + 0.025
          ...
          it("COMMIT->REFUND->COMPLAIN->FINALIZE->WITHDRAW",
            async () => {
              // 0.3
          ...
          it("COMMIT->REFUND->CANCEL->FINALIZE->WITHDRAW",
            async () => {
              // 0.3 + 0.04 + 0.025
              const expectedBuyerAmount =
                new BN(constants.buyer_deposit)
          ...
          it("COMMIT->REDEEM->COMPLAIN->CANCEL->FINALIZE->WITHDRAW",
            async () => {
              // 0.065
              const expectedBuyerAmount =
                new BN(constants.buyer_deposit)
                  .add(new BN(constants.seller_deposit)
                    .div(new BN(2)))
              // 0.3125
              const expectedSellerAmount =
                new BN(constants.product_price)
                  .add(new BN(constants.seller_deposit).div(new BN(4)))
              // 0.0125
              const expectedEscrowAmount =
                new BN(constants.seller_deposit).div(new BN(4))
      ...
      describe("TKN - TKN [WITH PERMIT]" +
        ...
        async () => {
          ...
          it("COMMIT->REDEEM->COMPLAIN->FINALIZE->WITHDRAW",
            async () => {
              const voucherID = await utils.commitToBuy(
                users.buyer,
                users.seller,
              ...
              const expectedEscrowAmountPrice = new BN(0)
              // 0.05
              const expectedEscrowAmountDeposit = new BN(constants.seller_deposit)
          ...
          it("COMMIT->REDEEM->CANCEL->FINALIZE->WITHDRAW",
            async () => {
              const voucherID = await utils.commitToBuy(
                users.buyer,
                users.seller,
      ...
      xdescribe("TKN - TKN SAME [WITH PERMIT]" +
        `${i === PAUSED_WITH_PERMIT ? PAUSED_LABEL : ''}`, async () => {
        ...
        async function getBalancesFromSameTokenContract () {
          balanceBuyer = await utils.contractBSNTokenSAME
            .balanceOf(users.buyer.address)
          balanceSeller = await utils.contractBSNTokenSAME
            .balanceOf(users.seller.address)
          escrowBalance = await utils.contractBSNTokenSAME
            .balanceOf(users.deployer.address)
          cashierBalance = await utils.contractBSNTokenSAME
            .balanceOf(utils.contractCashier.address)
        }
            ...
            assert.isTrue(
              escrowBalance.eq(
                expectedEscrowAmountPrice
                  .add(expectedEscrowAmountDeposit)),
      ...
      describe("ETH - TKN [WITH PERMIT]" +
        ...
        it("COMMIT->REFUND->COMPLAIN->CANCEL->FINALIZE->WITHDRAW",
          async () => {
            ...
            truffleAssert.eventEmitted(
              ...
              (ev) => {
                assert.equal(
                  ...
                  "Incorrect Payee")
                assert.isTrue(
                  ev._payment.eq(expectedBuyerPrice))
            ...
            assert.isTrue(
              balanceBuyerFromDeposits.eq(expectedBuyerDeposit),
              "Buyer did not get expected tokens from " +
  ...
  describe("[WHEN PAUSED] Seller withdraws deposit locked in escrow",
    async () => {
      let remQty = 10
      ...
      describe("ETH ETH", () => {
        ...
        it("[NEGATIVE] Should revert if called when contract is not paused",
          async () => {
            await truffleAssert.reverts(
              contractCashier.withdrawDeposits(
                TOKEN_SUPPLY_ID, {
                  from: users.seller.address
                }),
        ...
        it("[NEGATIVE] should revert if not called from the seller",
          async () => {
            await truffleAssert.reverts(
              contractCashier.withdrawDeposits(
                TOKEN_SUPPLY_ID, {
        ...
        it("Seller should be able to withdraw deposits for the " +
          "remaining QTY in Token Supply",
          async () => {
            let withdrawTx = await contractCashier
              .withdrawDeposits(
                TOKEN_SUPPLY_ID, {
                  from: users.seller.address
                })
        ...
        it("Escrow should have correct balance after burning the " +
          "rest of the supply",
          async () => {
            const expectedBalance =
              new BN(constants.seller_deposit)
                .mul(new BN(voucherToBuyBeforeBurn))
        ...
        it("Remaining QTY for Token Supply should be ZERO",
          async () => {
            let remainingQtyInContract = await contractVoucherKernel
              .getRemQtyForSupply(TOKEN_SUPPLY_ID, users.seller.address)
        ...
        it("[NEGATIVE] Buyer should not be able to commit to buy " +
        ...
        it("[NEGATIVE] Seller should not be able withdraw its deposit " +
      ...
      describe("[WITH PERMIT]", () => {
        describe("ETH_TKN", () => {
          ...
          it("Seller should be able to withdraw deposits for the " +
            ...
            async () => {
              ...
              truffleAssert.eventEmitted(
                ...
                (ev) => {
                  assert.equal(
                    ev.to, users.seller.address, "Incorrect Payee")
          ...
          it("[NEGATIVE] Seller should not be able withdraw its " +
        ...
        describe("TKN_ETH", () => {
          ...
          it("[NEGATIVE] Should revert if called when contract is " +
            "not paused",
            async () => {
              await truffleAssert.reverts(
                contractCashier.withdrawDeposits(
        ...
        describe("TKN_TKN", () => {
          ...
          it("[NEGATIVE] Should revert if called when contract " +
            "is not paused",
            async () => {
              await truffleAssert.reverts(
                contractCashier.withdrawDeposits(
