...
contract("FundLimitsOracle", async addresses => {
  ...
  describe('FundLimitsOracle interaction', () => {
    ...
    describe("ETH", () => {
      ...
      it("[NEGATIVE] Should revert if attacker tries to change ETH Limit",
        async () => {
          await truffleAssert.reverts(
            contractFundLimitsOracle.setETHLimit(
              FIVE_ETHERS, {
    ...
    describe("Token", () => {
      it("Owner should set Token Limit", async () => {
        await contractFundLimitsOracle
          .setTokenLimit(contractBSNTokenPrice.address, FIVE_TOKENS)

        expectedLimit = await contractFundLimitsOracle
          .getTokenLimit(contractBSNTokenPrice.address)
      ...
      it("[NEGATIVE] Should revert if attacker tries to change " +
        "Token Limit",
        async () => {
          await truffleAssert.reverts(
