...
(async() => {
    ...
    let txOrder = await voucherKernelContract_Buyer.refund(
        VOUCHER_ID, {gasLimit: '4000000'});
    
    const receipt = await txOrder.wait()
    
    let parsedEvent = await findEventByName(receipt, 'LogVoucherRefunded', '_tokenIdVoucher')
