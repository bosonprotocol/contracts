...
module.exports = async function() {
    ...
    const br = await BosonRouter.deploy(voucherKernel.address, erc1155erc721.address, flo.address, cashier.address)
    ...
    console.log("\n$ ERC1155ERC721", event.event, "approved VoucherKernel:", event.args._approved)

    tx = await erc1155erc721.setVoucherKernelAddress(voucherKernel.address)
    txReceipt = await tx.wait()
    event = txReceipt.events[0]
    console.log("\n$ ERC1155ERC721", event.event, "at:", event.args._newVoucherKernel)

    tx = await erc1155erc721.setBosonRouterAddress(br.address)
    txReceipt = await tx.wait() 
    event = txReceipt.events[0]
    console.log("\n$ ERC1155ERC721", event.event, "at:", event.args._newBosonRouter)

    tx = await voucherKernel.setBosonRouterAddress(br.address)
    txReceipt = await tx.wait()
    event = txReceipt.events[0]
    console.log("\n$ VoucherKernel", event.event, "at:", event.args._newBosonRouter)
