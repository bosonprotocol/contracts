...
describe('TEST SCENARIO 016 :: SELLER CREATES, BUYER COMMITS, REDEEMS & COMPLAINS', async function () {
  ...
  it('TEST SCENARIO 16 :: SELLER CREATE :: 1.0 Seller creates a voucher set', async function () {
    const timestamp = await Utils.getCurrTimestamp();
    voucherSetDetails = await sellerCreate(timestamp, users);
    await format(voucherSetDetails);
  });

  it('TEST SCENARIO 16 :: SELLER CREATE :: 1.1 VALIDATE VALID FROM', async function () {
    aql(voucherSetDetails['ValidFrom'], helpers.PROMISE_VALID_FROM);
  });

  it('TEST SCENARIO 16 :: SELLER CREATE :: 1.2 VALIDATE VALID TO', async function () {
    aql(voucherSetDetails['ValidTo'], helpers.PROMISE_VALID_TO);
  });

  it('TEST SCENARIO 16 :: SELLER CREATE :: 1.3 VALIDATE ORDER QUANTITY', async function () {
    aql(voucherSetDetails['nftSupply'], helpers.ORDER_QUANTITY1);
  });

  it('TEST SCENARIO 16 :: SELLER CREATE :: 1.4 VALIDATE SELLER', async function () {
    aql(voucherSetDetails['nftSeller'], users.seller.address);
  });

  it('TEST SCENARIO 16 :: SELLER CREATE :: 1.5 VALIDATE PAYMENT TYPE', async function () {
    aql(voucherSetDetails['paymentType'], 1);
  });

  it('TEST SCENARIO 16 :: SELLER CREATE :: 1.6 VALIDATE ERC1155ERC721 DATA', async function () {
  ...
  it('TEST SCENARIO 16 :: BUYER COMMITS :: 2.0 Buyer commits to purchases a voucher', async function () {
  ...
  it('TEST SCENARIO 16 :: BUYER COMMITS :: 2.1 VALIDATE ISSUER', async function () {
    aql(committedVoucher['issuer'], users.seller.address);
  });

  it('TEST SCENARIO 16 :: BUYER COMMITS :: 2.2 VALIDATE HOLDER', async function () {
    aql(committedVoucher['holder'], users.buyer.address);
  });

  it('TEST SCENARIO 16 :: BUYER REDEEMS :: 3.0 Buyer redeems a purchased voucher', async function () {
    console.log(await checkBalance(users));
  ...
  it('TEST SCENARIO 16 :: BUYER REDEEMS :: 3.1 VALIDATE REDEEMED VOUCHER', async function () {
    aql(
  ...
  it('TEST SCENARIO 16 :: SELLER FAULTS :: 4.0 Seller admits fault on a redeemed voucher', async function () {
    console.log(await checkBalance(users));
  ...
  it('TEST SCENARIO 16 :: SELLER FAULTS :: 4.1 VALIDATE FAULTED VOUCHER', async function () {
    aql(
