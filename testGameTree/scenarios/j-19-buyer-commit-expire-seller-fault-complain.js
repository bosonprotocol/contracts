...
describe('TEST SCENARIO 19 :: OFFER, COMMIT, EXPIRE, COF, COMPLAIN', async function () {
  ...
  it('TEST SCENARIO 19 :: OFFER :: 1.0 SELLER CREATES VOUCHER SET', async function () {
    const timestamp = await Utils.getCurrTimestamp();
    voucherSetDetails = await sellerCreate(timestamp, users);
    await format(voucherSetDetails);
  });

  it('TEST SCENARIO 19 :: OFFER :: 1.1 VALIDATE VALID FROM', async function () {
    aql(voucherSetDetails['ValidFrom'], helpers.PROMISE_VALID_FROM);
  });

  it('TEST SCENARIO 19 :: OFFER :: 1.2 VALIDATE VALID TO', async function () {
    aql(voucherSetDetails['ValidTo'], helpers.PROMISE_VALID_TO);
  });

  it('TEST SCENARIO 19 :: OFFER :: 1.3 VALIDATE ORDER QUANTITY', async function () {
    aql(voucherSetDetails['nftSupply'], helpers.ORDER_QUANTITY1);
  });

  it('TEST SCENARIO 19 :: OFFER :: 1.4 VALIDATE SELLER', async function () {
    aql(voucherSetDetails['nftSeller'], users.seller.address);
  });

  it('TEST SCENARIO 19 :: OFFER :: 1.5 VALIDATE PAYMENT TYPE', async function () {
    aql(voucherSetDetails['paymentType'], 1);
  });

  it('TEST SCENARIO 19 :: OFFER :: 1.6 VALIDATE ERC1155ERC721 DATA', async function () {
  ...
  it('TEST SCENARIO 19 :: COMMIT :: 2.0 BUYER COMMITS TO REDEEMING A VOUCHER', async function () {
  ...
  it('TEST SCENARIO 19 :: COMMIT :: 2.1 VALIDATE ISSUER', async function () {
    aql(commitVoucherDetails['issuer'], users.seller.address);
  });

  it('TEST SCENARIO 19 :: COMMIT :: 2.2 VALIDATE HOLDER', async function () {
    aql(commitVoucherDetails['holder'], users.buyer.address);
  });

  it('TEST SCENARIO 19 :: EXPIRE :: 3.0 LET VOUCHER EXPIRE', async function () {
    // fast-forward time
  ...
  it('TEST SCENARIO 19 :: EXPIRE :: 3.1 VALIDATE EXPIRY', async function () {
    aql(
  ...
  it('TEST SCENARIO 19 :: COF :: 5.0 SELLER ACCEPTS FAULT ON AN EXPIRED VOUCHER', async function () {
  ...
  it('TEST SCENARIO 19 :: COF :: 5.1 VALIDATE FAULTED VOUCHER', async function () {
    aql(
  ...
  it('TEST SCENARIO 19 :: COMPLAIN :: 6.0 BUYER COMPLAINS ABOUT EXPIRED VOUCHER', async function () {
    console.log(await checkBalance(users));
  ...
  it('TEST SCENARIO 19 :: COMPLAIN :: 6.1 VALIDATE COMPLAINED VOUCHER', async function () {
    aql(
