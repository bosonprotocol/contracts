...
describe('TEST SCENARIO 017 :: SELLER CREATES & BUYER COMMITS', async function () {
  ...
  it('TEST SCENARIO 17 :: SELLER CREATE :: 1.0 Seller creates a voucher set', async function () {
    const timestamp = await Utils.getCurrTimestamp();
    voucherSetDetails = await sellerCreate(timestamp, users);
    await format(voucherSetDetails);
  });

  it('TEST SCENARIO 17 :: SELLER CREATE :: 1.1 VALIDATE VALID FROM', async function () {
    aql(voucherSetDetails['ValidFrom'], helpers.PROMISE_VALID_FROM);
  });

  it('TEST SCENARIO 17 :: SELLER CREATE :: 1.2 VALIDATE VALID TO', async function () {
    aql(voucherSetDetails['ValidTo'], helpers.PROMISE_VALID_TO);
  });

  it('TEST SCENARIO 17 :: SELLER CREATE :: 1.3 VALIDATE ORDER QUANTITY', async function () {
    aql(voucherSetDetails['nftSupply'], helpers.ORDER_QUANTITY1);
  });

  it('TEST SCENARIO 17 :: SELLER CREATE :: 1.4 VALIDATE SELLER', async function () {
    aql(voucherSetDetails['nftSeller'], users.seller.address);
  });

  it('TEST SCENARIO 17 :: SELLER CREATE :: 1.5 VALIDATE PAYMENT TYPE', async function () {
    aql(voucherSetDetails['paymentType'], 1);
  });

  it('TEST SCENARIO 17 :: SELLER CREATE :: 1.6 VALIDATE ERC1155ERC721 DATA', async function () {
  ...
  it('TEST SCENARIO 17 :: BUYER COMMITS :: 2.0 Buyer commits to purchase a voucher', async function () {
  ...
  it('TEST SCENARIO 17 :: BUYER COMMITS :: 2.1 VALIDATE ISSUER', async function () {
    aql(commitVoucherDetails['issuer'], users.seller.address);
  });

  it('TEST SCENARIO 17 :: BUYER COMMITS :: 2.2 VALIDATE HOLDER', async function () {
    aql(commitVoucherDetails['holder'], users.buyer.address);
  });

  it('TEST SCENARIO 17 :: EXPIRE :: 3.0 Expire voucher', async function () {
    // fast-forward time
  ...
  it('TEST SCENARIO 17 :: CHECK EXPIRE :: 3.1 VALIDATE EXPIRY', async function () {
    aql(
  ...
  it('TEST SCENARIO 17 :: EXPIRE :: 4.0 CHECK EXPIRY STATUS', async function () {
  ...
  it('TEST SCENARIO 17 :: BUYER COMPLAINS :: 5.0 Buyer complains about redeemed voucher', async function () {
  ...
  it('TEST SCENARIO 17 :: BUYER COMPLAINS :: 5.1 VALIDATE COMPLAINED VOUCHER', async function () {
    aql(
